...
class ToyAVSimulator():
    ...
    def run_simulation(self, actions, s_0, simulation_horizon):
        ...
        while path_length < simulation_horizon:
            ...
            if self.collision_detected():
                # print("COLLISION: ", path_length, np.array(self._info))
    ...
    def collision_detected(self):
        ...
        if (np.any(np.all(np.less_equal(abs(dist), self.c_min_dist), axis=1)) and
                self._car[0] > 0.5):
            # print("Car induced crash")
            return True
        elif (np.any(np.all(np.less_equal(abs(dist), self.c_min_dist), axis=1)) and
                self._car[0] < 0.5):
            # print("Ped induced crash")
            return True
